...
const s = {
  ...
  statusChip: { borderRadius: 999, padding: '7px 10px', fontSize: 12, fontWeight: 800, whiteSpace: 'nowrap' },
  priceCard: {
    borderRadius: 16, border: '1px solid rgba(15,23,42,0.08)',
    background: '#f8fafc', padding: 14, marginBottom: 12,
  },
  ...
  payBtn: {
    ...
    textAlign: 'center', fontSize: 13, fontWeight: 600, color: '#1d4ed8',
    cursor: 'pointer',
  },
  availBar: {
  ...
  durationRow: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 6, marginBottom: 10 },
  durationBtn: {
    minHeight: 40, borderRadius: 12,
    border: '1px solid rgba(15,23,42,0.10)',
    background: '#ffffff', fontSize: 12, fontWeight: 800,
    cursor: 'pointer',
  },
  sessionTotal: {
  ...
  actionBtn: {
    minHeight: 44, padding: '10px 8px', borderRadius: 12,
...
export default function BayDetailSheet({
  ...
  useEffect(() => { setLocalStatus(bay.status); }, [bay.id, bay.status]);

  // Scroll to top when a new bay is opened
  useEffect(() => { scrollRef.current?.scrollTo({ top: 0 }); }, [bay.id]);
  ...
  return (
    <div style={s.overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div style={s.sheet} role="dialog" aria-modal="true" aria-labelledby="bay-title">
        {/* ── Fixed handle + close ─────────────────────────────────────── */}
        ...
        <div style={s.scrollBody} ref={scrollRef}>
          ...
          <div style={s.row}>
            <div>
              <div id="bay-title" style={s.streetName}>{bay.streetName || bay.name}</div>
          ...
          <div style={{ ...s.availBar, background: `${cfg.color}12`, border: `1px solid ${cfg.color}24` }}>
            <span style={{ ...s.availLabel, color: cfg.color }}>{cfg.label}</span>
          ...
          {bay.pricePerHour > 0 && (
            <div style={s.section}>
              ...
              <div style={s.durationRow} role="group" aria-label="Choose duration">
          ...
          <div style={s.actionRow}>
            {[
              { label: "I'm Leaving",   bg: '#f0fdf4', color: '#166534', border: '#bbf7d0',
                action: () => { setLocalStatus('free');  onBayStatusChange?.(bay.id,'free');  showToast('Bay marked as free'); } },
              { label: 'Paid',          bg: '#eff6ff', color: '#1d4ed8', border: '#bfdbfe',
                action: () => { setLocalStatus('paid');  onBayStatusChange?.(bay.id,'paid');  showToast('Bay marked as paid'); } },
              { label: "It's Full",     bg: '#fef2f2', color: '#991b1b', border: '#fecaca',
                action: () => { setLocalStatus('full');  onBayStatusChange?.(bay.id,'full');  showToast('Reported as full'); } },
              { label: 'Warden alert',  bg: '#fffbeb', color: '#92400e', border: '#fde68a',
                action: () => { onBayReport?.('warden', bay); showToast('Warden alert sent'); } },
            ].map(({ label, bg, color, border, action }) => (
              <button
                key={label}
                type="button"
                style={{ ...s.actionBtn, background: bg, color, border: `1px solid ${border}` }}
                onClick={action}
              >
                {label}
              </button>
